...
const scrape = async ({ parallel = false, from = 0, to = 1 } = {}) => {
  ...
  const proms = links.map((url) => async () => {
    ...
    const promises = Array.from(elements)
      .map((tr) => async () => {
        const tds = await tr.evaluate(async (tr: HTMLAnchorElement) => {
          const tds = Array.from(tr.querySelectorAll("td")).map(
            (td) => td.innerText
          );
          const baseUrl =
            "https://ifrafragrance.org/pdf/web/viewer.html?file=/standards/$name.pdf";
          const href = baseUrl.replace("$name", tr.getAttribute("data-id"));
          return [href, ...tds.slice(1)];
        });
        if (!tds || tds.length < 4) return tds;
...
export const store = (files) => {
  files.forEach(({ data: scraped, url }) => {
    ...
    fs.writeFile(
      pathNorm + "/" + name,
      JSON.stringify(cleaned.map(normalize as any).flat()),
      () => {
